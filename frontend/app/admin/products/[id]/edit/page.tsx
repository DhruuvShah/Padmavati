...
export default function EditProductPage() {
  ...
  const [rateMode, setRateMode] = useState<"code" | "direct">("code");
  ...
  async function loadProductData() {
    ...
    try {
      ...
      if (prod.rate_type === "per_kg") {
        if (prod.rate_code_id) { setRateMode("code"); setRateCodeId(prod.rate_code_id); }
        else { setRateMode("direct"); setDirectRate(prod.direct_rate || ""); }
      } else {
        setRateMode("direct");
    ...
    <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-200 pb-20">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          <Button variant="ghost" size="icon" aria-label="Back to Products" onClick={() => router.push("/admin/products")} className="h-8 w-8 text-muted-foreground">
            <ArrowLeft className="h-4 w-4" />
          </Button>
          <div>
            <h1 className="text-3xl sm:text-4xl font-display font-semibold text-foreground tracking-tight">Edit Product</h1>
            <p className="text-muted-foreground mt-2 text-sm sm:text-base">Update your product details and pricing</p>
          </div>
        </div>
        <div className="flex flex-wrap items-center gap-2 sm:gap-3 w-full sm:w-auto">
          ...
          <Button variant="outline" size="sm" onClick={() => router.push("/admin/products")} className="flex-1 sm:flex-none">Cancel</Button>
        ...
        <div className="p-4 sm:p-8 space-y-8">
            ...
            <div className="flex-1 space-y-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <SearchableCombobox ariaLabel="Select category" options={categories.map(c => ({ id: c.id, label: c.name, tags: c.tags || [] }))} value={categoryId} onChange={setCategoryId} onCreateNew={createCategory} placeholder="Search category..." />
                </div>
                <div className="space-y-2">
                  <Label id="party-label">Party (Optional)</Label>
                  <SearchableCombobox ariaLabel="Select party" options={parties.map(p => ({ id: p.id, label: p.code_name }))} value={partyId} onChange={setPartyId} onCreateNew={createParty} placeholder="Search party..." />
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6">
            <div className="space-y-2">
              ...
              <Input id="weightKg" value={weightKg} onChange={(e) => setWeightKg(e.target.value)} type="text" inputMode="decimal" pattern="^\d*\.?\d{0,3}$" placeholder="0.000" />
            </div>
            <div className="space-y-2">
              <Label htmlFor="heightInches">Height (inches)</Label>
              <Input id="heightInches" value={heightInches} onChange={(e) => setHeightInches(e.target.value)} type="text" inputMode="decimal" placeholder="Optional" />
            </div>
            <div className="space-y-2">
              <Label htmlFor="lengthInches">Length (inches)</Label>
              <Input id="lengthInches" value={lengthInches} onChange={(e) => setLengthInches(e.target.value)} type="text" inputMode="decimal" placeholder="Optional" />
            </div>
          </div>
            ...
            <fieldset className="space-y-3">
              ...
              <div className="flex flex-col sm:flex-row gap-4 sm:gap-6">
                <label className="flex items-center gap-2 cursor-pointer">
                  <input type="radio" name="rateType" value="per_kg" checked={rateType === "per_kg"} onChange={() => setRateType("per_kg")} className="accent-primary" />
                  <span className="text-sm font-medium">Per Kg</span>
                </label>
                <label className="flex items-center gap-2 cursor-pointer">
                  <input type="radio" name="rateType" value="per_piece" checked={rateType === "per_piece"} onChange={() => setRateType("per_piece")} className="accent-primary" />
            ...
            <div className="p-6 liquid-glass border-transparent border border-border rounded-3xl">
              {rateType === 'per_kg' ? (
                <div className="space-y-4">
                  ...
                  {rateMode === 'code' ? (
                    <div className="max-w-sm space-y-2">
                      ...
                      <SearchableCombobox ariaLabel="Select rate code" options={rateCodes.filter(r => r.rate_type === 'per_kg').map(r => ({ id: r.id, label: r.code }))} value={rateCodeId} onChange={setRateCodeId} placeholder="Search code..." />
                    </div>
                  ) : (
                    <div className="max-w-sm space-y-2">
                      ...
                      <div className="relative">
                        ...
                        <Input id="product-rate-kg-direct" value={directRate} onChange={(e) => setDirectRate(e.target.value)} type="text" inputMode="decimal" className="pl-7" placeholder="0.00" />
                      </div>
                    </div>
                  )}
                </div>
                ...
                <div className="max-w-sm space-y-2">
                  ...
                  <div className="relative">
                    <span className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground">₹</span>
                    <Input id="product-rate-piece-direct" value={directRate} onChange={(e) => setDirectRate(e.target.value)} type="text" inputMode="decimal" className="pl-7" placeholder="0.00" />
                  </div>
                </div>
              )}
            </div>
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <Button variant="outline" size="sm" onClick={() => setVariants([...variants, { weightKg: "", rateCodeId: "", directRate: "", heightInches: "", lengthInches: "" }])}>
                  ...
                  <div key={index} className="flex gap-4 items-end p-4 liquid-glass border-transparent relative pr-12 rounded-3xl">
                    ...
                    <div className="flex-1 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                      <div className="space-y-2">
                        ...
                        <Input id={`variant-${index}-weight`} type="number" step="any" min="0" onWheel={(e) => (e.target as HTMLElement).blur()} placeholder="0.000" value={variant.weightKg} onChange={(e) => { const v = [...variants]; v[index].weightKg = e.target.value; setVariants(v); }} />
                      </div>
                      <div className="space-y-2">
                        <div className="text-xs text-muted-foreground uppercase font-bold tracking-wider mb-2">Rate Mode</div>
                        {rateMode === 'code' ? (
                          <SearchableCombobox ariaLabel="Select rate code" options={rateCodes.filter(r => r.rate_type === rateType).map(r => ({ id: r.id, label: r.code }))} value={variant.rateCodeId} onChange={(val) => { const v = [...variants]; v[index].rateCodeId = val; v[index].directRate = ""; setVariants(v); }} />
                        ) : (
                          <div className="relative">
                            <span className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground">₹</span>
                            <Input id={`variant-${index}-directRate`} aria-label={`Variant ${index} direct rate`} type="number" step="any" min="0" onWheel={(e) => (e.target as HTMLElement).blur()} placeholder="0.00" className="pl-7" value={variant.directRate} onChange={(e) => { const v = [...variants]; v[index].directRate = e.target.value; v[index].rateCodeId = ""; setVariants(v); }} />
                          </div>
                        )}
                      </div>
                      <div className="space-y-2">
                        <Label htmlFor={`variant-${index}-height`} className="text-xs text-muted-foreground uppercase font-bold tracking-wider">H (in)</Label>
                        <Input id={`variant-${index}-height`} type="number" step="any" min="0" onWheel={(e) => (e.target as HTMLElement).blur()} placeholder="-" value={variant.heightInches} onChange={(e) => { const v = [...variants]; v[index].heightInches = e.target.value; setVariants(v); }} />
                      </div>
                      <div className="space-y-2">
                        <Label htmlFor={`variant-${index}-length`} className="text-xs text-muted-foreground uppercase font-bold tracking-wider">L (in)</Label>
                        <Input id={`variant-${index}-length`} type="number" step="any" min="0" onWheel={(e) => (e.target as HTMLElement).blur()} placeholder="-" value={variant.lengthInches} onChange={(e) => { const v = [...variants]; v[index].lengthInches = e.target.value; setVariants(v); }} />
                      </div>
                    </div>
                  </div>
                ))}
